"""
Framework = este practic o biblioteca (library) care ofera sprijin pentru
constriuirea unui website sau unui web API

Un wb framework este mai conplex decat o librarie gen requests/random/...
...
CLI (command line interface) pentru rularea unui server, conexiunea la db si alte comenzi
- decoratori, design patterns

"""
from flask import Flask, redirect, url_for

# Cream o aplicatie Flask, careia ii dam numele __name__ -> numele fisierului nostru
app = Flask(__name__)   # __name__ nume fisier curent
...
flask --app .\S8\ST1\web_framework.py run

Generic vorbind:
flasjk --app <nume fisier python unde avem app> run

Vom primi o linie care ne zice ca serverul s-a pornit:
 * Running on http://127.0.0.1:5000

127.0.0.1 = adresa IP a calculatorului nostru in  reteua personala
5000 = portul pe care ruleaza serverul nostru; un calculator are multe porturi si trebuie sa precizam portul
atunci cand rulam app
"""

"""
Debug = atunci cand avem o eroare de logica in cod se numeste bug,
Debug = tehnica si actiunile pe care noi le facem ca sa depistam si sa reparam un bug.

Cea mai simpla metoda de debugging este sa printam anumite variabile in codul nostru
si sa urmarim firul logic de executie pana cand depistam linia la care se intampla eroarea
...
"""
Pentru a putea avea parametrii in endpoint, folosim semnele < >
...
Pnetru aceasta ruta, acum putem apela:
http://127.0.0.1:5000/hello/baa_bau


In cazul in care anumitor caractere din URL, browserul le va inlocui cu niste coduri, spre exemplu spatiul ( )
...
DAR
http://127.0.0.1:5000/users/Octav va da eroare
"""
...
"""
Redirecting: va redirectiona un request catre alt endpoint
In FLask, folosim url_for pentru a redeirectiona un reuqest catre un alt endpoint (functie)
"""
...
def hello_user(user_id):
    ...
    if user_id in users:
        ...
        return "Not valid user! Please register!", 404
